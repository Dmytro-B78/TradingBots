...
def main():
    ...
    print("\n=== ТЕСТ: kill-switch ===")
    trades = [
        {"Time": datetime.now(prague_tz).strftime("%Y-%m-%d %H:%M:%S"), "Symbol": "BTC/USDT", "Profit(%)": -3, "Profit(USDT)": -30},
        {"Time": datetime.now(prague_tz).strftime("%Y-%m-%d %H:%M:%S"), "Symbol": "ETH/USDT", "Profit(%)": -4, "Profit(USDT)": -40},
        {"Time": datetime.now(prague_tz).strftime("%Y-%m-%d %H:%M:%S"), "Symbol": "BNB/USDT", "Profit(%)": -5, "Profit(USDT)": -50},
    ]
    for t in trades:
